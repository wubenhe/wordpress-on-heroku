			...
			if ($count > 0) {
				...
				if (!count($required)) {
					...
					if ($auto_responder) {
						...
						$emailed = @wp_mail( $postEmail, html_entity_decode($subject, ENT_QUOTES), $auto_copy, $headers );
					}
					
					// admin notifier
					if ($admin_notify) {
						$site_title = urldecode(get_bloginfo('name'));
						...
						if (! empty($custom_fields) ) $copy .= implode('<br/><br/>',$custom_fields);
						$emailed = @wp_mail( $admin_email, urldecode($subject), stripslashes($copy), $headers );
					}
